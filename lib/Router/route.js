...
function Route(method, path, rawrules, callbacks, options) {
  options = options || {};
  var self = this;
  this.path = path.substring(0, path.indexOf("?")) || path;
  this.method = method;
...
Route.prototype.match = function(path, req){
  ...
  for (var param in this.rules.params){
    if(this.rules.params[param] === ""){
      if (!(req.body && req.body.hasOwnProperty(param) 
          || req.query.hasOwnProperty(param)))

        return false;
    }
    else if (req.param(param) !== this.rules.params[param])
      return false;
  }
